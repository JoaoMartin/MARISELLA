...
namespace CapaPresentacion
{
    public partial class frmIngresoAlmacen : Form
    {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (registrar)
                    {
                        ...
                        if(txtNroUnidades.Text.Trim().Length == 0)
                        {
                            nroUnidades = 00.00m;
                        }else
                        {
                            nroUnidades = Convert.ToDecimal(txtNroUnidades.Text.Trim());
                        }
                        row["NroUnidades"] = nroUnidades;
                        row["Monto"] = monto;
        ...
        private void Total()
        {
            ...
            {
                total = total + Convert.ToDecimal(dataListadoDetalle.Rows[i].Cells[7].Value.ToString());
            }
        ...
        private void btnEditar_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (registrar)
                {
                    ...
                    this.dataListadoDetalle[3, Convert.ToInt32(this.lblPosic.Text)].Value = nroUnidades.ToString();
                    if(txtMonto.Text.Trim() == string.Empty)
                    {
                        this.dataListadoDetalle[7, Convert.ToInt32(this.lblPosic.Text)].Value = "00.00";
                    }else
                    {
                        this.dataListadoDetalle[7, Convert.ToInt32(this.lblPosic.Text)].Value = this.txtMonto.Text.Trim();
                    }
                    this.dataListadoDetalle.ClearSelection();
                    this.Total();
                    this.limpiarDetalle();
        ...
        private void dataListadoDetalle_DoubleClick(object sender, EventArgs e)
        {
            if (this.lblBander.Text.Equals("1") && (this.btnGuardar.Enabled == true) && (this.dataListadoDetalle.Rows.Count > 0))
            {
                ...
                this.txtNroUnidades.Text = Convert.ToString(this.dataListadoDetalle.CurrentRow.Cells["NroUnidades"].Value);
                this.txtPrecioUnitario.Text = Convert.ToString(this.dataListadoDetalle.CurrentRow.Cells["PrecioUnitario"].Value);
        ...
        private void txtNroUnidades_KeyPress(object sender, KeyPressEventArgs e)
        {
            if ((int)e.KeyChar == (int)Keys.Enter)
            {
                this.Agregar();
            ...
            for (int i = 0; i < txtNroUnidades.Text.Length; i++)
            {
                if (txtNroUnidades.Text[i] == '.')
                    IsDec = true;
