...
namespace CapaPresentacion
{
    public partial class frmCierreCaja : Form
    {
        ...
        public void total()
        {
            ...
            decimal abono = 00.00m;
            decimal devolucion = 00.00m;
            decimal anulada = 00.00m;
            decimal credito = Convert.ToDecimal(txtCredito.Text);
            decimal cortesia = Convert.ToDecimal(txtCortesia.Text);
            ...
            devolucion = Convert.ToDecimal(lblDevolucion.Text);
            anulada = Convert.ToDecimal(lblAnulada.Text);

            txtCredito.Text = (credito - anulada).ToString();

            txtEnEfectivo.Text = (ventasEfectivo - devolucion).ToString();
            totalParcial = abono +ventasEfectivo + otrosIngreso - egresos;
            montoApertura = Convert.ToDecimal(lblMontoInicial.Text);
            totalCaja = totalParcial + montoApertura;
            totalVenta = ventasEfectivo + ventasTarjeta - devolucion + credito + cortesia;
        ...
        public void ventas()
        {
            ...
            if (dt.Rows[13][0].ToString() != "")
            {
                this.txtAbono.Text = dt.Rows[13][0].ToString();
            }
            else
            {
                this.txtAbono.Text = "00.00";
            }
            if (dt.Rows[14][0].ToString() != "")
            {
                this.lblDevolucion.Text = dt.Rows[14][0].ToString();
            }
            else
            {
                this.lblDevolucion.Text = "00.00";
            }
            if (dt.Rows[15][0].ToString() != "")
            {
                this.lblAnulada.Text = dt.Rows[15][0].ToString();
            }
            else
            {
