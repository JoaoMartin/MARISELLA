...
{
    public partial class frmRComprobantesAnulados : Form
    {
        public frmRComprobantesAnulados()
        {
            InitializeComponent();
        }

        private void frmRComprobantesAnulados_Load(object sender, EventArgs e)
        {
            try
            {
                string bandera = frmMostrarComprobanteAnulado.f1.lblBandera.Text;
                string fechaInicio = "";
                string fechaFin = "";
                DateTime fecIn;
                if (bandera == "1")
                {
                    fechaInicio = frmMostrarComprobanteAnulado.f1.dtpFechaInicio.Value.ToString("yyyy-MM-dd" + " 00:00:00");
                    fechaFin = frmMostrarComprobanteAnulado.f1.dtpFechaFin.Value.ToString("yyyy-MM-dd" + " 23:59:59");
                }
                else
                {
                ...
                repdoc.Load(@"D:\Reportes\RComprobantesAnulados.rpt");
