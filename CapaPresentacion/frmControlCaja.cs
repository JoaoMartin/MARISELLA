...
namespace CapaPresentacion
{
    public partial class frmControlCaja : Form
    {
        ...
        public void ventas()
        {
            ...
            {
                this.lblAnuladas.Text = dt.Rows[15][0].ToString();
            }
            else
            {
                this.lblAnuladas.Text = "00.00";
            }
            if (dt.Rows[16][0].ToString() != "")
            {
                this.lblNotaPedido.Text = dt.Rows[16][0].ToString();
            }
            else
            {
                this.lblNotaPedido.Text = "00.00";
            }
            //cuenta_usuario.idCliente(txtUsuario.Text, txtContraseña.Text);
            // dt.Rows[0]["Estado"].ToString();
            decimal total = 00.00m;
            total = total + Convert.ToDecimal(txtMontoApertura.Text) + Convert.ToDecimal(txtVentaEfectivo.Text) + Convert.ToDecimal(txtOtrosIngresos.Text) -
                Convert.ToDecimal(txtSalidas.Text) + Convert.ToDecimal(txtAbono.Text) - Convert.ToDecimal(lblAnuladas.Text);
