        ...
        private void btnPagar_Click(object sender, EventArgs e)
        {
            ...
            if (opcion == DialogResult.OK)
            {
                ...
                if (this.lblIdVenta.Text == "0")
                {
                    ...
                    frmVenta.f1.dtDetalleMenu, DateTime.Now, 00.00m, Convert.ToInt32(this.lblIdUsuario.Text), "", "", "", "","",00.00m);
                    if (rpta != "")
                    {
                        for (int i = 0; i < frmVenta.f1.dataListadoDetalle.Rows.Count; i++)
                        {
                            ...
                            DataTable dtReceta = NReceta.Mostrar(Convert.ToInt32(frmVenta.f1.dataListadoDetalle.Rows[i].Cells["Cod"].Value.ToString()));
