            ...
            if (this.lblBandera.Text == "0")
            {
                ...
                if (rpta == "OK")
                {
                    ...
                    if (rpta == "OK")
                    {
                        ...
                        {
                            if(frmMostrarVentas.f1.lblForma.Text == "CREDITO")
                            {
                                NCaja.Insertar(Convert.ToInt32(frmPrincipal.f1.lblIdUsuario.Text), "1", "EGRESO", Convert.ToDecimal(frmMostrarVentas.f1.lblAdelanto.Text),
                              "ANULACION", "EFECTIVO");
                            }else if (frmMostrarVentas.f1.lblForma.Text == "EFECTIVO")
                            {
                                NCaja.Insertar(Convert.ToInt32(frmPrincipal.f1.lblIdUsuario.Text), "1", "EGRESO", Convert.ToDecimal(frmMostrarVentas.f1.lblTotalVenta.Text),
                             "ANULACION", "EFECTIVO");
                            }
                            else if (frmMostrarVentas.f1.lblForma.Text == "MIXTO")
                            {
                                NCaja.Insertar(Convert.ToInt32(frmPrincipal.f1.lblIdUsuario.Text) ,"1", "EGRESO", Convert.ToDecimal(frmMostrarVentas.f1.lblEfectivo.Text),
                             "ANULACION", "EFECTIVO");
                            }
            ...
            else
            {
                ...
                if (rpta == "OK")
                {
                    ...
                    if (rpta == "OK")
                    {
                        string tipoDoc;
                        DataTable dtCorrelativo = NComprobanteAnulado.mostrarCorrelativo(DateTime.Now);
                        DateTime fechaGen = Convert.ToDateTime(lblIdCompra.Text);
                        string fechaConv = fechaGen.ToString("yyyy-MM-dd");
